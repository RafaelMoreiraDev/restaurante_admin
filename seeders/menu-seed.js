...
async function seed() {
    try {
      ...
      for (const categoriaNome in MENU) {
        if (MENU.hasOwnProperty(categoriaNome)) {
          const categoriaItens = MENU[categoriaNome];
          ...
          // Insere os itens da categoria
          for (const item of categoriaItens) {
              // Cria o item associado à categoria encontrada ou criada
              await Item.create({
                  nome: item.name,
                  descricao: item.dsc,
                  preco: item.price,
